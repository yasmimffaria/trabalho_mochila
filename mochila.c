...
typedef struct tipoMochila
{
    ...
    float razao[MAXTAM];     // raẓo entre o beneficio e o peso
} tipoMochila;
...
// Traz a mochila de um arquivo, e guarda na struct que ̩ usada de retorno
tipoMochila criar_mochila(char *StrEntrada)
{
    ...
    do
    {
        ...
        // # - Ignorar linha
        case '#':
            /* L̻ o restante da linha, at̩ chegar no final de linha '\n' */
            do
            {
...
void enumeracao(tipoMochila mochila, int vetOtimo[], float *peso_otimo, float *beneficio_otimo)
{
    ...
    *peso_otimo = 0;
    ...
    printf("\n");
    printf("Este ̩ o melhor subconjunto pelo metodo da Enumeracao");
    printf("\n");
    for (int i = 0; i < mochila.N; i++)
    {
        printf("%d ", vetOtimo[i]);
